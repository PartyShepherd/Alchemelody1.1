...
# ------------------ Alchemelody Alarm/Planetary Hours ------------------
...
def index():
    try:
        ...
        location = {"latitude":40.4406,"longitude":-79.9959}  # default
...
def get_elemental_quarter():
    ...
    if 0 <= current_hour < 6: return "Earth (12 AM - 6 AM)"
    elif 6 <= current_hour < 12: return "Air (6 AM - 12 PM)"
    elif 12 <= current_hour < 18: return "Fire (12 PM - 6 PM)"
    else: return "Water (6 PM - 12 AM)"
...
def get_moon_phase():
    try:
        ...
        elif phase < 0.25: return "Waxing Crescent"
        elif phase < 0.27: return "First Quarter"
        elif phase < 0.50: return "Waxing Gibbous"
        elif phase < 0.53: return "Full Moon"
        elif phase < 0.75: return "Waning Gibbous"
        elif phase < 0.77: return "Last Quarter"
        else: return "Waning Crescent"
    except: return "Moon phase data unavailable"
...
def get_planetary_hour_planner():
    try:
        sunrise, sunset = get_sunrise_sunset()
        if not sunrise or not sunset: return "Planetary hour data unavailable"
        planets_list = ["Sun", "Venus", "Mercury", "Moon", "Saturn", "Jupiter", "Mars"]
        start_planet = {"Sunday":"Sun","Monday":"Moon","Tuesday":"Mars","Wednesday":"Mercury",
        ...
        if sunrise <= current_time < sunset:
            ...
            current_planet = planets_list[(planets_list.index(day_start_planet) + hours_since_sunrise) % 7]
        else:
            hours_since_sunset = int((current_time - sunset) / nighttime_duration)
            current_planet = planets_list[(planets_list.index(day_start_planet) + 12 + hours_since_sunset) % 7]
...
def planner():
    ...
    weather = get_weather()
    planetary_hour = get_planetary_hour_planner()
