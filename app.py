...
def calculate_planetary_hours(latitude, longitude):
    ...
    try:
        ...
        sunrise = datetime.utcfromtimestamp(data["sys"]["sunrise"]).replace(tzinfo=pytz.utc).astimezone(tz)
        sunset = datetime.utcfromtimestamp(data["sys"]["sunset"]).replace(tzinfo=pytz.utc).astimezone(tz)
        ...
        for i in range(12):
            hours.append({"time_str": current_time.strftime("%I:%M %p"), "hour": current_time.hour, "minute": current_time.minute, "planet": planets[i%7]})
            current_time += day_hour
        for i in range(12):
            hours.append({"time_str": current_time.strftime("%I:%M %p"), "hour": current_time.hour, "minute": current_time.minute, "planet": planets[(i+12)%7]})
...
def index():
    location = {"latitude": 40.4406, "longitude": -79.9959}  # Default Pittsburgh
...
def play_sound():
    ...
    if not planet:
        return {"message":"No planet provided"}, 400
    print(f"Playing sound for {planet}")
    return {"message":f"Playing sound for {planet}"}, 200
